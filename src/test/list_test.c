...
void copy_list_test(void **state) {
    ...
    List copy_same;
    ctor_list(&copy_same);
    copy_list(&list, &copy_same);
    List copy_not_same;
    ctor_list(&copy_not_same);
    copy_list(&list, &copy_not_same);
    for (int i = 0; i < list.length; i++) {
        assert(copy_same.length > i);
        assert(copy_not_same.length > i);
        assert(copy_same.array.data[i].bits == list.array.data[i].bits);
        assert(copy_not_same.array.data[i].bits == list.array.data[i].bits);
    }
    dtor_list(&list);
    dtor_list(&copy_same);
    dtor_list(&copy_not_same);
}
...
int run_list_tests() {
    ...
    set_array(&tests, 10, from_ptr(copy_list_test));
    set_array(&tests, 11, from_ptr(add_list_test));
    set_array(&tests, 12, from_ptr(iter_list_test));
    set_array(&tests, 13, from_ptr(dtor_list_test));
